...
public final class StorageCache {
    ...
    StorageCache(StorageUnit storageUnit, BlockMenu menu) {
        ...
        menu.addMenuClickHandler(STATUS_SLOT, (p, slot, item, action) -> {
            Bukkit.getScheduler().runTaskLater(InfinityExpansion.getInstance(), () -> {
                if (removing) {
                    return;
                }
    ...
    void tick(Block block) {
        ...
        if (InfinityExpansion.slimefunTickCount() % 20 == 0) {
            final Block signBlock = getSignBlock(block);
            if (signBlock != null) {
    ...
    public static Block getSignBlock(Block storageUnit) {
        final Block sign = storageUnit.getRelative(BlockFace.UP);
        if (Tag.SIGNS.isTagged(sign.getType())) {
            return sign;
        } else {
            for (BlockFace blockFace : List.of(BlockFace.NORTH, BlockFace.EAST, BlockFace.SOUTH, BlockFace.WEST)) {
                final Block wallSign = storageUnit.getRelative(blockFace);
                if (Tag.WALL_SIGNS.isTagged(wallSign.getType()) && wallSign.getRelative(((WallSign) wallSign.getBlockData()).getFacing().getOppositeFace()).equals(storageUnit)) {
                    return wallSign;
                }
            }
        }
